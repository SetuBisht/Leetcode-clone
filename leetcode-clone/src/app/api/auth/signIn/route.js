...
export const POST = async (req, res, next) => {
  try {
    ...
    const verifiedUser = await getUserByEmail(email).select(
    ...
    if (verifiedUser.authentication && verifiedUser.authentication.salt) {
      const expectedHash = authentication(
        verifiedUser.authentication.salt,
        password
      );
      ...
      if (expectedHash !== verifiedUser.authentication.password) {
        ...
        const salt = random();
        verifiedUser.authentication.sessionToken = authentication(
          salt,
        ...
        response.cookies.set(
          ...
          verifiedUser.authentication.sessionToken,
          {
            httpOnly: true,
            path: "/",
          }
